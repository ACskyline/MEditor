...
void Mesh::pushPullPP(Face* f, glm::vec3 d, float theta, glm::vec3 p, glm::vec3 n)
{
    ...
    do
    {
        ...
        f->nor = glm::dot(f->nor, ntemp) > 0 ? ntemp : -ntemp;
    ...
    else
    {
        ...
        if(intersectPlanePlane(f, p, n, &origin, &direction))
        {
            ...
            for(auto item : Vev)
            {
                ...
                qDebug() << "nPotential:" << nPotential.x << "," << nPotential.y << "," << nPotential.z <<
                            " pPotential:" << pPotential.x << "," << pPotential.y << "," << pPotential.z <<
                            " parallel:" << parellel;
...
void Mesh::cleanUp(Face *f)
{
    ...
    do{
        ...
        HalfEdge* subCurrent = current->next->sym->next;//check adjacent vertices except for the one connected with current
    ...
    {
        qDebug() << "current:" << current->id;
        HalfEdge* currentNext = current->next;
        HalfEdge* currentNextNext = current->next->next;
        HalfEdge* currentSym = current->sym;
        Vertex* currentVertex = current->vert;
        Vertex* nextVertex = current->next->vert;
    ...
    qDebug() << "cleanUp edge to vertex case";
...
int Mesh::mergeVertex(Vertex *vKeep, Vertex *vDiscard, HalfEdge* KeepToDiscard)
{
    ...
    HalfEdge* current = start;
    do{
        qDebug() << "merge current:" << current->id;
        qDebug() << "merge currentSym:" << current->sym->id;
    ...
    qDebug() << "mergeVertex finish, deleted triangle ? :" << result;
